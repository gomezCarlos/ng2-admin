...
export class ResponsiveTable implements OnInit{
  ...
  getPage(page : number){
    this.service.getPage(page).subscribe(response => {this.projects = response;},error => {this.error = error; if(error.status == 403)error.statusText="Usuario no autorizado"; if(error.status==401)error.statusText="Usuario no autorizado."; console.log(error)})
  }
